...
```ts
async function main() {
  const project = await braintrust.project.create({ name: 'name' }).catch(async (err) => {
...
await braintrust.project.create({ name: 'name' }, {
  maxRetries: 5,
});
```
...
const { data: project, response: raw } = await braintrust.project.create({ name: 'name' }).withResponse();
