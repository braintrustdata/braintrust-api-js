...
describe('resource function', () => {
  ...
  test('create: required and optional params', async () => {
    const response = await client.function.create({ function_data: { type: 'prompt' }, name: 'name', project_id: '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e', slug: 'slug', description: 'description', function_type: 'task', origin: { object_type: 'organization', object_id: '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e', internal: true }, prompt_data: { prompt: { type: 'completion', content: 'content' }, options: { model: 'model', params: { use_cache: true, temperature: 0, top_p: 0, max_tokens: 0, frequency_penalty: 0, presence_penalty: 0, response_format: { type: 'json_object' }, tool_choice: 'auto', function_call: 'auto', n: 0, stop: ['string', 'string', 'string'] }, position: 'position' }, parser: { type: 'llm_classifier', use_cot: true, choice_scores: { foo: 0 } }, origin: { prompt_id: 'prompt_id', project_id: 'project_id', prompt_version: 'prompt_version' } }, tags: ['string', 'string', 'string'] });
  });
  ...
  test('update: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.function.update('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e', { description: 'description', function_data: { type: 'prompt' }, name: 'name', prompt_data: { prompt: { type: 'completion', content: 'content' }, options: { model: 'model', params: { use_cache: true, temperature: 0, top_p: 0, max_tokens: 0, frequency_penalty: 0, presence_penalty: 0, response_format: { type: 'json_object' }, tool_choice: 'auto', function_call: 'auto', n: 0, stop: ['string', 'string', 'string'] }, position: 'position' }, parser: { type: 'llm_classifier', use_cot: true, choice_scores: { foo: 0 } }, origin: { prompt_id: 'prompt_id', project_id: 'project_id', prompt_version: 'prompt_version' } }, tags: ['string', 'string', 'string'] }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Braintrust.NotFoundError);
  });
