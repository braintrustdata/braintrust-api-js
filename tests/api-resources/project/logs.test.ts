...
describe('resource logs', () => {
  test('feedback: only required params', async () => {
    const responsePromise = client.project.logs.feedback('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e', {
  ...
  test('feedback: required and optional params', async () => {
    const response = await client.project.logs.feedback('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e', {
      feedback: [
        {
          id: 'id',
          comment: 'comment',
  ...
  test('fetch: request options instead of params are passed correctly', async () => {
    ...
    await expect(
      client.project.logs.fetch('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e', { path: '/_stainless_unknown_path' }),
  ...
  test('fetchPost: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      client.project.logs.fetchPost('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e', {
        path: '/_stainless_unknown_path',
      }),
    ).rejects.toThrow(Braintrust.NotFoundError);
  });
  ...
  test('insert: only required params', async () => {
    const responsePromise = client.project.logs.insert('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e', {
      events: [{}, {}, {}],
    });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
  ...
  test('insert: required and optional params', async () => {
    const response = await client.project.logs.insert('182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e', {
      events: [
        {
          id: 'id',
          _is_merge: true,
