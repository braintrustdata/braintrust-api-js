...
describe('instantiate client', () => {
  ...
  describe('defaultHeaders', () => {
    const client = new Braintrust({ baseURL: 'http://localhost:5000/', defaultHeaders: { 'X-My-Default-Header': '2' }, apiKey: 'My API Key' })
  ...
  describe('defaultQuery', () => {
    test('with null query params given', () => {
      const client = new Braintrust({ baseURL: 'http://localhost:5000/', defaultQuery: { apiVersion: 'foo' }, apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null)).toEqual('http://localhost:5000/foo?apiVersion=foo');
    });

    test('multiple default query params', () => {
      const client = new Braintrust({ baseURL: 'http://localhost:5000/', defaultQuery: { apiVersion: 'foo', hello: 'world' }, apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null)).toEqual('http://localhost:5000/foo?apiVersion=foo&hello=world');
    });

    test('overriding with `undefined`', () => {
      const client = new Braintrust({ baseURL: 'http://localhost:5000/', defaultQuery: { hello: 'world' }, apiKey: 'My API Key' })
      expect(client.buildURL('/foo', { hello: undefined })).toEqual('http://localhost:5000/foo');
    });
  });

  test('custom fetch', async () => {
    const client = new Braintrust({ baseURL: 'http://localhost:5000/', apiKey: 'My API Key', fetch: (url) => {
  ...
  test('custom signal', async () => {
    const client = new Braintrust({ baseURL: process.env["TEST_API_BASE_URL"] ?? 'http://127.0.0.1:4010', apiKey: 'My API Key', fetch: (...args) => {
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['BRAINTRUST_BASE_URL'] = undefined;
    });
...
describe('retries', () => {
  ...
  test('retry count header', async () => {
    ...
    const client = new Braintrust({ apiKey: 'My API Key', fetch: testFetch, maxRetries: 4 });
